...
_Generated from the live test suite (2026-05-09). Single source of truth: re-run `node tools/gen-catalog.js` after adding tests._

## Summary

- **Total: 214 tests** across 30 spec files (97 UI · 117 API)
- **Tagged with `@everstage-qa`: 145** assignment tests
- **Documented vulnerabilities / UX findings**: 26 (asserted as actual Juice Shop behavior — see [Findings](#findings-against-the-default-juice-shop-build))
...
| @negative | 42 | Bad input is rejected; errors are surfaced |
| @security | 42 | SQLi, XSS, JWT, IDOR/BOLA, mass-assignment, oversized payloads |
...
| Task | Tag | Count |
|---|---|---|
| Task 1 — Login + beforeEach | `@task1` | 15 |
| Task 2 — UI add card | `@task2` | 23 |
| Task 3 — API add card | `@task3` | 46 |
...
| regression | `@regression` | 98 | Full nightly slice |
| e2e | `@e2e` | 14 | End-to-end user journeys |
| functional | `@functional` | 63 | Behavior tests |
| nonfunctional | `@nonfunctional` | 12 | Latency, masking, robustness |
...
### Payment Cards — UI (Task 2)

_ui/add-card.spec.ts · 23 tests_
...
### Payment Cards — API (Task 3)

_api/add-card.spec.ts · 46 tests_
...
| `TC-API-110` | Missing cardNum is accepted on default Juice Shop | Low / UX | `api/add-card.spec.ts` |
| `TC-API-111` | Missing expMonth is accepted on default Juice Shop | Low / UX | `api/add-card.spec.ts` |
| `TC-API-112` | Missing expYear is accepted on default Juice Shop | Low / UX | `api/add-card.spec.ts` |
| `TC-API-115` | POST with an empty body is accepted on default Juice Shop | Low / UX | `api/add-card.spec.ts` |
...
| `TC-API-150` | re-posting an identical card is accepted (no idempotency / no de-dup) | Low / UX | `api/add-card.spec.ts` |
| `TC-API-1503` | security-question endpoint is an enumeration oracle | Medium | `api/whoami.spec.ts` |
| `TC-API-153` | PATCH on /api/Cards/{id} crashes with 500 instead of returning 405 | Low / UX | `api/add-card.spec.ts` |
...
| `TC-UI-016` | whitespace-only cardholder name passes form validation | Low / UX | `ui/add-card.spec.ts` |
| `TC-UI-120` | SQL-injection email comment bypass logs in on default Juice Shop | High | `ui/login.spec.ts` |
